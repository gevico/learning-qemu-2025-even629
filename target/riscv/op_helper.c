...
{
	/*
    qemu_log("UMA: ENTER - dst=0x%lx, src=0x%lx, grain=%lu\n",
             dst, src, grain_size);
    */
	int grain = size & 0x3;
	int block_size;
	...
	/*
    qemu_log("UMA: Processing %dx%d matrix transpose\n",
             block_size, block_size);
    */

	for (int i = 0; i < block_size; i++) {
		for (int j = 0; j < block_size; j++) {
			target_ulong src_offset = (j * block_size + i) * sizeof(uint32_t);
			target_ulong dest_offset = (i * block_size + j) * sizeof(uint32_t);
			uint32_t value = cpu_ldl_data(env, src + src_offset);
			cpu_stl_data(env, dest + dest_offset, value);
		}
	}
}
// rd rs1 rs2
void helper_sort(CPURISCVState *env, target_ulong sort_num, target_ulong address, target_ulong array_num)
{
	...
	int i, j, swapped, a, b;
	int n = sort_num;
	target_ulong a_addr, b_addr;

	// 冒泡排序
	for (i = 0; i < n - 1; i++) {
		swapped = 0;
		for (j = 0; j < n - i - 1; j++) {
			a_addr = address + sizeof(int32_t) * j;
			b_addr = address + sizeof(int32_t) * (j + 1);
			a = cpu_ldl_data(env, a_addr);
			b = cpu_ldl_data(env, b_addr);
			if (a > b) {
				// 交换
				cpu_stl_data(env, a_addr, b);
				cpu_stl_data(env, b_addr, a);
				swapped = 1;
			}
		}
		if (!swapped) {
...
void helper_crush(CPURISCVState *env, target_ulong dest_addr, target_ulong src_addr, target_ulong size)
{
	size_t i = 0, j = 0;
	uint8_t curr_data, next_data;

	while (i + 1 < size) {
		curr_data = cpu_ldub_data(env, src_addr + i);
		next_data = cpu_ldub_data(env, src_addr + i + 1);
		cpu_stb_data(env, dest_addr + j, (curr_data & 0xf) | ((next_data & 0xf) << 4));
		i += 2;
		j++;
	}
	...
	if (i < size) {
		curr_data = cpu_ldub_data(env, src_addr + i);
		cpu_stb_data(env, dest_addr + j, (curr_data & 0xf));
		j++;
	}
}

void helper_expand(CPURISCVState *env, target_ulong dest_addr, target_ulong src_addr, target_ulong size)
{
	size_t i = 0, j = 0;
	uint8_t curr_data;
	...
	for (i = 0; i < size; i++) {
		curr_data = cpu_ldub_data(env, src_addr + i);
		cpu_stb_data(env, dest_addr + j++, (curr_data & 0xf));
		cpu_stb_data(env, dest_addr + j++, ((curr_data >> 4) & 0xf));
	}
}
...

{
	...
	trace_riscv_exception(exception, riscv_cpu_get_trap_name(exception, false), env->pc);
...
target_ulong helper_csrrw(CPURISCVState *env, int csr, target_ulong src, target_ulong write_mask)
{
	target_ulong val = 0;
...
void helper_csrw_i128(CPURISCVState *env, int csr, target_ulong srcl, target_ulong srch)
{
	RISCVException ret = riscv_csrrw_i128(env, csr, NULL, int128_make128(srcl, srch), UINT128_MAX, GETPC());
...
target_ulong helper_csrrw_i128(CPURISCVState *env, int csr, target_ulong srcl, target_ulong srch, target_ulong maskl,
			       target_ulong maskh)
{
	Int128 rv = int128_zero();
	RISCVException ret =
		riscv_csrrw_i128(env, csr, &rv, int128_make128(srcl, srch), int128_make128(maskl, maskh), GETPC());
 ...
 */
static void check_zicbo_envcfg(CPURISCVState *env, target_ulong envbits, uintptr_t ra)
{
	...
	if (env->virt_enabled && (((env->priv <= PRV_S) && !get_field(env->henvcfg, envbits)) ||
 ...
 * fault (virtualized).
 */
static void check_zicbom_access(CPURISCVState *env, target_ulong address, uintptr_t ra)
{
	RISCVCPU *cpu = env_archcpu(env);
	int mmu_idx = riscv_env_mmu_index(env, false);
     ...
     */
	ret = probe_access_flags(env, address, cbomlen, MMU_DATA_LOAD, mmu_idx, true, &phost, ra);
...
target_ulong helper_sret(CPURISCVState *env)
{
	...
	if (!riscv_cpu_allow_16bit_insn(&env_archcpu(env)->cfg, env->priv_ver, env->misa_ext) && (retpc & 0x3)) {
	...
	if (riscv_cpu_cfg(env)->ext_ssdbltrp) {
		if (riscv_has_ext(env, RVH)) {
			target_ulong prev_vu = get_field(env->hstatus, HSTATUS_SPV) && prev_priv == PRV_U;
	...
	if (riscv_cpu_cfg(env)->ext_smctr || riscv_cpu_cfg(env)->ext_ssctr) {
		riscv_ctr_add_entry(env, env->pc, retpc, CTRDATA_TYPE_EXCEP_INT_RET, src_priv, src_virt);
	}

	return retpc;
}

static void check_ret_from_m_mode(CPURISCVState *env, target_ulong retpc, target_ulong prev_priv, uintptr_t ra)
{
	if (!(env->priv >= PRV_M)) {
		riscv_raise_exception(env, RISCV_EXCP_ILLEGAL_INST, ra);
	}
	...
	if (riscv_cpu_cfg(env)->pmp && !pmp_get_num_rules(env) && (prev_priv != PRV_M)) {
		riscv_raise_exception(env, RISCV_EXCP_INST_ACCESS_FAULT, ra);
	}
}
static target_ulong ssdbltrp_mxret(CPURISCVState *env, target_ulong mstatus, target_ulong prev_priv,
				   target_ulong prev_virt)
{
	/* If returning to U, VS or VU, sstatus.sdt = 0 */
	if (prev_priv == PRV_U || (prev_virt && (prev_priv == PRV_S || prev_priv == PRV_U))) {
		mstatus = set_field(mstatus, MSTATUS_SDT, 0);
...
target_ulong helper_mret(CPURISCVState *env)
{
	...
	target_ulong prev_virt = get_field(env->mstatus, MSTATUS_MPV) && (prev_priv != PRV_M);
	mstatus = set_field(mstatus, MSTATUS_MIE, get_field(mstatus, MSTATUS_MPIE));
	mstatus = set_field(mstatus, MSTATUS_MPIE, 1);
	mstatus = set_field(mstatus, MSTATUS_MPP, riscv_has_ext(env, RVU) ? PRV_U : PRV_M);
	...
	if (riscv_cpu_cfg(env)->ext_smctr || riscv_cpu_cfg(env)->ext_ssctr) {
		riscv_ctr_add_entry(env, env->pc, retpc, CTRDATA_TYPE_EXCEP_INT_RET, PRV_M, false);
	}
...
void helper_ctr_add_entry(CPURISCVState *env, target_ulong src, target_ulong dest, target_ulong type)
{
	riscv_ctr_add_entry(env, src, dest, (enum CTRType)type, env->priv, env->virt_enabled);
}
...
void helper_wfi(CPURISCVState *env)
{
	...
	if (((prv_s || (!rvs && prv_u)) && get_field(env->mstatus, MSTATUS_TW)) ||
		...
		riscv_raise_exception(env, RISCV_EXCP_ILLEGAL_INST, GETPC());
	} else if (env->virt_enabled && (prv_u || (prv_s && get_field(env->hstatus, HSTATUS_VTW)))) {
		riscv_raise_exception(env, RISCV_EXCP_VIRT_INSTRUCTION_FAULT, GETPC());
	} else {
		cs->halted = 1;
...
{
	if (env->virt_enabled && (env->priv == PRV_S || env->priv == PRV_U) && get_field(env->hstatus, HSTATUS_VTW) &&
...
void helper_tlb_flush(CPURISCVState *env)
{
	CPUState *cs = env_cpu(env);
	if (!env->virt_enabled &&
	    (env->priv == PRV_U || (env->priv == PRV_S && get_field(env->mstatus, MSTATUS_TVM)))) {
		riscv_raise_exception(env, RISCV_EXCP_ILLEGAL_INST, GETPC());
	} else if (env->virt_enabled && (env->priv == PRV_U || get_field(env->hstatus, HSTATUS_VTVM))) {
		riscv_raise_exception(env, RISCV_EXCP_VIRT_INSTRUCTION_FAULT, GETPC());
	} else {
		tlb_flush(cs);
	}
...
{
	if (env->priv == PRV_S && !env->virt_enabled && get_field(env->mstatus, MSTATUS_TVM)) {
		riscv_raise_exception(env, RISCV_EXCP_ILLEGAL_INST, GETPC());
	}
